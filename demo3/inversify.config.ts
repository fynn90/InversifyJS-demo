...
// 容器可以用来绑定 class直接的关系
let warriorModule = new ContainerModule((bind: interfaces.Bind, unbind: interfaces.Unbind) => {
  bind<Warrior>(TYPES.Warrior).to(Ninja)
})

let weaponModule = new ContainerModule((bind: interfaces.Bind,unbind:interfaces.Unbind) => {
  bind<Weapon>(TYPES.Weapon).to(Katana)
})

let throwableWeaponModule = new ContainerModule((bind:interfaces.Bind, unbind:interfaces.Unbind) => {
